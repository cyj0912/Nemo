...
FGLSLProgram*
FGLSLProgram::CreateFromFiles(const char* vertFilename, const char* fragFilename, const char* geomFilename)
{
    FGLSLProgram* prog = new FGLSLProgram();
...
FGLSLProgram* FGLSLProgram::CreateFromShaders(FGLSLVertexShader* vs, FGLSLPixelShader* ps)
{
    FGLSLProgram* prog = new FGLSLProgram();
    if (!prog->SetFromShaders(vs, ps))
    {
        delete prog;
        return nullptr;
    }
...
bool FGLSLProgram::SetFromShaders(FGLSLVertexShader* vs, FGLSLPixelShader* ps)
{
    assert(vs->IsCompiled() && ps->IsCompiled());
    ...
    vs->AttachTo(GLProgram);
    ps->AttachTo(GLProgram);
